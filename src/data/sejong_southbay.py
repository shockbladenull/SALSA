...
class SejongSouthbayLoader(Dataset):  # 定义一个继承自 Dataset 的类 SejongSouthbayLoader
    def __init__(self, all_file_loc=None, pcl_transform=None):  # 初始化方法，接受文件路径列表和点云变换函数作为参数
        if all_file_loc == None:  # 如果文件路径列表为 None
            with open('/home/ljc/Dataset/Apollo-Southbay/train_southbay_2_10.pickle', 'rb') as file:  # 打开 Southbay 数据集文件
                self.southbay_data_dict = pickle.load(file)  # 加载 Southbay 数据集字典
            with open('/home/ljc/Dataset/Mulran/Sejong/train_Sejong1_Sejong2_2_10.pickle', 'rb') as file:  # 打开 Sejong 数据集文件
            ...
            for i in range(len(self.southbay_data_dict)):  # 遍历 Southbay 数据字典
                self.all_file_loc.append(self.southbay_data_dict[i].rel_scan_filepath)  # 添加文件路径
                self.transforms.append(self.southbay_data_dict[i].pose)  # 添加变换矩阵
                self.pos_pairs_ind.append(self.southbay_data_dict[i].positives.tolist())  # 添加正样本对索引
                self.non_negative_pairs_ind.append(self.southbay_data_dict[i].non_negatives.tolist())  # 添加非负样本对索引
            ...
            for i in range(len(self.sejong_data_dict)):  # 遍历 Sejong 数据字典
                self.all_file_loc.append(self.sejong_data_dict[i].rel_scan_filepath)  # 添加文件路径
                self.transforms.append(self.sejong_data_dict[i].pose)  # 添加变换矩阵
                pos_pairs = (np.array(self.sejong_data_dict[i].positives) + len_southbay).tolist()  # 计算正样本对索引
                self.pos_pairs_ind.append(pos_pairs)  # 添加正样本对索引
                non_neg_pairs = (np.array(self.sejong_data_dict[i].non_negatives) + len_southbay).tolist()  # 计算非负样本对索引
    ...
    def data_prepare(self, xyzr, voxel_size=np.array([0.1, 0.1, 0.1])):  # 数据准备方法，接受点云数据和体素大小作为参数
        ...
        hash_vals, _, uniq_idx = self.sparse_quantize(coords, return_index=True, return_hash=True)  # 稀疏量化坐标，返回哈希值和唯一索引
    ...
    def sparse_quantize(self, coords, voxel_size=1, return_index=False, return_inverse=False, return_hash=False):  # 稀疏量化方法
        if isinstance(voxel_size, (float, int)):  # 如果体素大小是浮点数或整数
            voxel_size = tuple(repeat(voxel_size, 3))  # 将体素大小重复三次，形成元组
        assert isinstance(voxel_size, tuple) and len(voxel_size) == 3  # 断言体素大小是长度为 3 的元组

        voxel_size = np.array(voxel_size)  # 将体素大小转换为 numpy 数组
        coords = np.floor(coords / voxel_size).astype(np.int32)  # 计算体素化后的坐标，并转换为 int32 类型

        hash_vals, indices, inverse_indices = np.unique(self.ravel_hash(coords), return_index=True, return_inverse=True)  # 计算唯一哈希值和索引
        coords = coords[indices]  # 获取唯一索引对应的坐标

        if return_hash: outputs = [hash_vals, coords]  # 如果返回哈希值，则输出哈希值和坐标
        else: outputs = [coords]  # 否则只输出坐标

        if return_index:  # 如果返回索引
            outputs += [indices]  # 添加索引到输出
        if return_inverse:  # 如果返回逆索引
            outputs += [inverse_indices]  # 添加逆索引到输出
        return outputs[0] if len(outputs) == 1 else outputs  # 如果输出长度为 1，则返回第一个元素，否则返回输出列表
    ...
    def __getitem__(self, idx):  # 获取数据方法
        ...
        if len(xyzr) > 0:  # 如果点云数据不为空
            coords, xyz, feats = self.data_prepare(xyzr, voxel_size=np.array([self.voxel_size, self.voxel_size, self.voxel_size]))  # 准备数据
        else:
...
class SejongSouthbayTupleLoader(SejongSouthbayLoader):  # 定义一个继承自 SejongSouthbayLoader 的类 SejongSouthbayTupleLoader
    def __init__(self, cached_queries=1000, pcl_transform=None):  # 初始化方法，接受缓存查询数量和点云变换函数作为参数
        ...
        self.prev_epoch_neg = (-np.ones((len(self.all_file_loc), self.nNeg), dtype=int)).tolist()  # 初始化上一轮训练的负样本索引
    ...
    def update_subcache(self, net, outputdim):  # 更新子缓存方法
        ...
        nidxs = np.random.choice(neg_smapling_set, self.cached_queries * 4, replace=False)  # 随机选择负样本索引
        ...
        dNeg_arr, n_ind_arr = faiss_index.search(qvecs, self.nNeg + 1)  # 搜索最接近查询的 nNeg+1 个负样本
        # dNeg_arr - 距离矩阵
        # n_ind_arr - 对应的索引
    ...
    def __getitem__(self, idx):  # 获取数据方法
        anchor_filename, pos_filename, neg_filename = self.triplets[idx][0]  # 获取三元组的文件路径
        ...
        anchor_coords, anchor_xyz, anchor_feats = self.data_prepare(anchor_xyzr, voxel_size=np.array([self.voxel_size, self.voxel_size, self.voxel_size]))  # 准备锚点数据

        pos_xyzr = self.read_pcd_file(pos_filename)  # 读取正样本点云数据
        pos_coords, pos_xyz, pos_feats = self.data_prepare(pos_xyzr, voxel_size=np.array([self.voxel_size, self.voxel_size, self.voxel_size]))  # 准备正样本数据

        point_pos_pairs = self.get_point_tuples(anchor_xyz, pos_xyz, anchor_idx, pos_idx, anchor_filename)  # 获取匹配的点对

        neg_xyzr = self.read_pcd_file(neg_filename)  # 读取负样本点云数据
        neg_coords, neg_xyz, neg_feats = self.data_prepare(neg_xyzr, voxel_size=np.array([self.voxel_size, self.voxel_size, self.voxel_size]))  # 准备负样本数据

        return anchor_coords, anchor_xyz, anchor_feats, pos_coords, pos_xyz, pos_feats, neg_coords, neg_xyz, neg_feats, labels, point_pos_pairs  # 返回锚点、正样本和负样本的数据及标签和匹配的点对
