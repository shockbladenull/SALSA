...
def generate_triplets(
    ...
    print(
        f"{len(pc_ids)} point clouds left from {len(pc_poses)} after filtering with min_displacement={min_displacement}"
    )
    ...
    triplets = [
        Triplet(
            e.anchor,
            [p for p in e.positives if p in anchors_set],
            [nn for nn in e.non_negatives if nn in anchors_set],
        )
        for e in triplets
    ]
    print(len(triplets))
    ...
    for triplet in triplets:
        ...
        non_negatives = np.array(
            [new_ids[e] for e in triplet.non_negatives], dtype=np.int32
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate training triplets for Apollo SouthBay dataset"
    )
    parser.add_argument(
        "--dataset_root",
        type=str,
        default="/data/raktim/Datasets/Apollo-Southbay",
        help="Path to Apollo SouthBay root folder",
    )
    ...
    triplets = generate_triplets(
        ...
        "MapData",
        query_split,
        positives_th=args.pos_th,
        negatives_th=args.neg_th,
