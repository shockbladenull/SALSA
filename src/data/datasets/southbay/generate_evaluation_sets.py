...
def get_scans(
    ds: SouthBayDataset, split: str, area: str, min_displacement: float = 0.1
    ...
    for ndx in ds.location_ndx[split][area]:
        ...
        item = EvaluationTuple(
            timestamp, rel_scan_filepath, position=position, pose=pose
        )
        elems.append(item)
    ...
    print(
        f"{len(elems)} filtered elements in {split} split with grid cell size = {min_displacement}"
    )
...
def generate_evaluation_set(
    ds: SouthBayDataset, area: str, min_displacement: float = 0.1, dist_threshold=5
) -> EvaluationSet:
    ...
    print(
        f"Area: {area} - {len(map_set)} database elements, {len(query_set)} query elements\n"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate evaluation sets for Apollo SouthBay dataset"
    )
    parser.add_argument(
        "--dataset_root",
        type=str,
        required=False,
        default="/data/raktim/Datasets/Apollo-Southbay",
    )
    ...
    print(
        f"Minimum displacement between scans in each set (map/query): {args.min_displacement}"
    )
    print(
        f"Ignore query elements without a corresponding map element within a threshold [m]: {args.dist_threshold}"
    )
    ...
    eval_set = generate_evaluation_set(
        ds, area, min_displacement=min_displacement, dist_threshold=args.dist_threshold
    )
    pickle_name = (
        f"test_{area}_{args.min_displacement}_{args.dist_threshold}_20m.pickle"
    )
