...
def read_lidar_poses(
    poses_filepath: str, lidar_filepath: str, pose_time_tolerance: float = 1.0
):
    ...
    all_lidar_timestamps = [
        int(os.path.splitext(f)[0])
        for f in os.listdir(lidar_filepath)
    ...
    lidar_poses = np.array(
        lidar_poses, dtype=np.float64
    )  # (northing, easting) position

    print(
        f"{len(lidar_timestamps)} scans with valid pose, {count_rejected} rejected due to unknown pose"
    )
