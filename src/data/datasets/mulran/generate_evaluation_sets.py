...
def get_scans(
    sequence: MulranSequence, ts_range: tuple = None
) -> List[EvaluationTuple]:
    # Get a list of all readings from the test area in the sequence
    elems = []
    for ndx in range(len(sequence)):
        if ts_range is not None:
            if (ts_range[0] > sequence.timestamps[ndx]) or (
                ts_range[1] < sequence.timestamps[ndx]
            ):
                continue
...
def generate_evaluation_set(
    dataset_root: str,
    map_sequence_name: str,
    query_sequence_name: str,
    min_displacement: float = 0.2,
    dist_threshold=20,
) -> EvaluationSet:
    split = "test"
    map_sequence = MulranSequence(
        dataset_root, map_sequence_name, split=split, min_displacement=min_displacement
    )
    query_sequence = MulranSequence(
        dataset_root,
        query_sequence_name,
        split=split,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate evaluation sets for Mulran dataset"
    )
    parser.add_argument(
        "--dataset_root",
        type=str,
        required=False,
        default="/data/raktim/Datasets/Mulran/Sejong",
    )
    ...
    for map_sequence, query_sequence in sequences:
        ...
        test_set = generate_evaluation_set(
            args.dataset_root,
            map_sequence,
            query_sequence,
