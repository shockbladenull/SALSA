...
class Kitti360Sequence(Dataset):
    ...
    def __init__(
        ...
        self.pose_file = os.path.join(
            self.dataset_root, self.sequence_name, "poses.txt"
        )
        self.calib_file = os.path.join(
            self.dataset_root, self.sequence_name, "cam0_to_world.txt"
        )
        assert os.path.exists(
            self.pose_file
        ), f"Cannot access sequence pose file: {self.pose_file}"
        self.times_file = os.path.join(
            self.dataset_root, self.sequence_name, "velodyne_points/timestamps.txt"
        )
        assert os.path.exists(
            self.pose_file
        ...
        self.rel_scan_filepath = [
            os.path.join(self.rel_lidar_path, "%010d%s" % (e, ".bin"))
            for e in filenames
        ]
        print("")
