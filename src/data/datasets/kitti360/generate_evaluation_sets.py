...
def get_scans(
    sequence: Kitti360Sequence, min_displacement: float = 0.1, ts_range: tuple = None
    ...
    for ndx in range(len(sequence)):
        if ts_range is not None:
            if (ts_range[0] > sequence.rel_lidar_timestamps[ndx]) or (
                ts_range[1] < sequence.rel_lidar_timestamps[ndx]
            ):
                continue
        ...
        item = EvaluationTuple(
            sequence.rel_lidar_timestamps[ndx],
            sequence.rel_scan_filepath[ndx],
            position,
            pose,
        )
        elems.append(item)
    ...
    print(
        f"{count_skipped} clouds skipped due to displacement smaller than {min_displacement}"
    )
...
def generate_evaluation_set(
    ...
    query_set = get_scans(
        sequence,
        min_displacement,
        (MAP_TIMERANGE[-1], sequence.rel_lidar_timestamps[-1]),
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate evaluation sets for KItti dataset"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="/data/raktim/Datasets/KITTI360/KITTI-360/data_3d_raw",
    )
    ...
    kitti_eval_set = generate_evaluation_set(
        args.dataset_root,
        sequence,
    ...
    file_path_name = os.path.join(
        ...
        f"kitti360_{sequence}_{args.min_displacement}_eval.pickle",
    )
