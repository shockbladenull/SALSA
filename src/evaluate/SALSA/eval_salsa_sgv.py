...
class Evaluator:
    def __init__(
        ...
        self.eval_set_filepath = os.path.join(
        ...
        assert os.path.exists(
            self.eval_set_filepath
        ), f"Cannot access evaluation set pickle: {self.eval_set_filepath}"
    ...
    def evaluate(self, model, *args, **kwargs):
        ...
        for query_ndx in tqdm.tqdm(query_indexes):
            ...
            tp = {
                r: [
                    tp[r][nn] + (1 if (euclid_dist[: nn + 1] <= r).any() else 0)
        ...
        recall = {
            r: [tp[r][nn] / self.n_samples for nn in range(self.k)] for r in self.radius
        }
    ...
    def compute_embeddings(
        ...
        for ndx, e in tqdm.tqdm(enumerate(eval_subset)):
            ...
            if embeddings is None:
                embeddings = np.zeros(
                    (len(eval_subset), embedding.shape[1]), dtype=embedding.dtype
                )
...
def euclidean_distance(query, database):
    return (
        torch.cdist(
            torch.tensor(query).unsqueeze(0).unsqueeze(0),
            torch.tensor(database).unsqueeze(0),
        )
        .squeeze()
        .numpy()
    )
...
class MetLocEvaluator(Evaluator):
    ...
    def evaluate(self, model, d_thresh, *args, **kwargs):
        ...
        for query_ndx in tqdm.tqdm(query_indexes):
            ...
            if d_thresh > 0:
                ...
                for k in range(topk):
                    k_id = nn_ndx[k]
                    conf_val = sgv_fn(
                        local_query_embeddings[query_ndx],
                        local_map_embeddings[k_id],
                        d_thresh=d_thresh,
                    )
            ...
            global_metrics["tp"] = {
                r: [
                    global_metrics["tp"][r][nn]
                    + (1 if (euclid_dist[: nn + 1] <= r).any() else 0)
                    for nn in range(self.k)
                ]
                for r in self.radius
            }
            global_metrics["tp_rr"] = {
                r: [
                    global_metrics["tp_rr"][r][nn]
                    + (1 if (euclid_dist_rr[: nn + 1] <= r).any() else 0)
                    for nn in range(self.k)
                ]
                for r in self.radius
            }
            global_metrics["RR"] = {
                r: global_metrics["RR"][r]
                + [
                    next(
                        (1.0 / (i + 1) for i, x in enumerate(euclid_dist <= r) if x), 0
                    )
                ]
                for r in self.radius
            }
            global_metrics["RR_rr"] = {
                r: global_metrics["RR_rr"][r]
                + [
                    next(
                        (1.0 / (i + 1) for i, x in enumerate(euclid_dist_rr <= r) if x),
                        0,
                    )
                ]
                for r in self.radius
            ...
            for eval_mode in ["Initial", "Re-Ranked"]:
                ...
                # Ransac alignment
                tick = time()
                T_estimated, inliers, _ = self.ransac_fn(
                    local_query_embeddings[query_ndx], local_map_embeddings[nn_idx]
                )
                ...
                metrics[eval_mode]["repeatability"].append(
                    calculate_repeatability(
                        kp1, kp2, T_gt, threshold=self.repeat_dist_th
                    )
                )
                metrics[eval_mode]["repeatability_refined"].append(
                    calculate_repeatability(
        ...
        global_metrics["recall"] = {
            r: [global_metrics["tp"][r][nn] / self.n_samples for nn in range(self.k)]
            for r in self.radius
        }
        global_metrics["recall_rr"] = {
            r: [global_metrics["tp_rr"][r][nn] / self.n_samples for nn in range(self.k)]
            for r in self.radius
        }
        global_metrics["MRR"] = {
            r: np.mean(np.asarray(global_metrics["RR"][r])) for r in self.radius
        }
        global_metrics["MRR_rr"] = {
            r: np.mean(np.asarray(global_metrics["RR_rr"][r])) for r in self.radius
        }
    ...
    def ransac_fn(self, query_keypoints, candidate_keypoints):
        ...
        ransac_result = get_ransac_result(
            query_keypoints["features"], candidate_keypoints["features"], kp1, kp2
        )
        return (
            ransac_result.transformation,
            len(ransac_result.correspondence_set),
    ...
    def data_prepare(self, xyzr, voxel_size=np.array([0.1, 0.1, 0.1])):
        ...
        hash_vals, _, uniq_idx = sparse_quantize(
    ...
    def compute_embeddings(self, eval_subset: List[EvaluationTuple], model):
        ...
        for ndx, e in tqdm.tqdm(enumerate(eval_subset), total=len(eval_subset)):
            ...
            coords, xyz, feats, points, batch_number = (
                coords.cuda(),
                xyz.cuda(),
                feats.cuda(),
                points.cuda(),
            ...
            with torch.inference_mode():
                global_embedding, keypoints, key_embeddings = self.compute_embedding(
                    model, [coords, xyz, feats, batch_number], points, is_dense=True
                )
            ###########################################################################################

            if global_embeddings is None:
                global_embeddings = np.zeros(
                    (len(eval_subset), global_embedding.shape[1]),
                    dtype=global_embedding.dtype,
                )

            global_embeddings[ndx] = global_embedding
            local_embeddings.append(
                {"keypoints": keypoints, "features": key_embeddings}
            )
    ...
    def compute_embedding(self, model, batch_data, points, is_dense=False):
        ...
        return (
            global_embedding,
            output_points.clone().detach().cpu(),
            torch.tensor(output_features, dtype=torch.float).cpu(),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        default="mulran",
        choices=["mulran", "southbay", "kitti", "alita", "kitti360"],
    )
    parser.add_argument(
        "--mulran_sequence",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[5, 20],
        help="True Positive thresholds in meters",
    )
    parser.add_argument(
        "--n_samples",
        type=int,
        default=None,
        help="Number of elements sampled from the query sequence",
    )
    parser.add_argument("--weights", type=str, default="/logg3d.pth")
    parser.add_argument(
        "--model", type=str, default="logg3d1k", choices=["logg3d", "logg3d1k"]
    )
    parser.add_argument(
        "--d_thresh", type=float, default=0.4, help="Dist thresholds in meters"
    )
    parser.add_argument(
        "--n_topk", type=int, default=20, help="Dist thresholds in meters"
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="If False, also evaluates metric localization",
    )
    ...
    # ####################### SALSA + PCA #################################################################
    model = CombinedModel(
        voxel_sz=args.voxel_size, num_in_features=512, num_out_features=256
    )
    # salsa_model_save_path = os.path.join(os.path.dirname(__file__),'../../checkpoints/SALSA/Model/model_30.pth')
    salsa_model_save_path = os.path.join(
        os.path.dirname(__file__), "../../checkpoints/SALSA/Model/", args.salsa_model
    )
