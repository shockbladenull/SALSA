...
class SejongSouthbayLoader(Dataset):
    def __init__(self, sequence, sample_size=-1):
        if sequence == "southbay":
            with open(
                "/data/raktim/Datasets/Apollo-Southbay/train_southbay_2_10.pickle", "rb"
            ) as file:
            ...
            with open(
                "/data/raktim/Datasets/Mulran/Sejong/train_Sejong1_Sejong2_2_10.pickle",
                "rb",
            ) as file:
        ...
        if sample_size != -1:
            self.all_file_loc = random.sample(
                self.all_file_loc, min(sample_size, len(self.all_file_loc))
            )
        self.voxel_size = 0.5
...
def do_pca(
    ...
    out_vecs = torch.zeros(
        (len(train_loader.dataset), num_in_features), dtype=torch.float32
    )
    count = 0
    ...
    pca_model.pca_conv.weight = nn.Parameter(
    ...
    if return_features_pca:
        ...
        feature_pca_model.pca_conv.weight = nn.Parameter(
            torch.from_numpy(np.expand_dims(np.expand_dims(u.T, -1), -1))
        )
...
def main():
    ...
    pca, pca_feature = do_pca(
        ...
        sequence="all",
        sample_size=-1,
        return_features_pca=return_feature_pca,
    )
    torch.save(
        pca.state_dict(),
        os.path.join(
    ...
    if return_feature_pca:
        torch.save(
            pca_feature.state_dict(),
            os.path.join(
                os.path.dirname(__file__),
                "../checkpoints/SALSA/PCA/pca_feature_model.pth",
            ),
        )
