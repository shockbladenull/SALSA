...
class SALSA(nn.Module):
    def __init__(self, voxel_sz):
        ...
        patch_size = np.array([voxel_size[i] * patch_size for i in range(3)]).astype(
            np.float32
        )
        window_size = patch_size * 6
        self.do_pe = True
        self.feature_extractor = Semantic(
            input_c=config["feat_extractor"]["input_c"],
            ...
            quant_size_sphere=np.array(config["feat_extractor"]["window_size_sphere"])
            / 24,
            rel_query=True,
            rel_key=True,
            rel_value=True,
    ...
    def forward(self, coord, xyz, feat, batch, save_attn_weights=False):
        ...
        split_local_fetures = torch.split(
            local_features, list(counts)
        )  # [(N1,16),(N2,16),(N3,16),(N4,16),...]
        for features in split_local_fetures:
            # print(features.shape)
            if save_attn_weights:
                attval, attn_weights = self.attpool(
                    features.unsqueeze(0), return_weights=True
                )
                self.attn_weights = attn_weights
            else:
                attval = self.attpool(features.unsqueeze(0))
            padded_split_local_features.append(
                attval.squeeze(0)
