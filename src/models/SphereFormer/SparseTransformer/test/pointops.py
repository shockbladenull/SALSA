...
class PrecomputeIndexPairs(Function):
    ...
    def forward(ctx, p2v_map, counts, index_0_offsets):
        ...
        """
        assert (
            p2v_map.is_contiguous()
            and counts.is_contiguous()
            and index_0_offsets.is_contiguous()
        )
        n, k = p2v_map.shape
        ...
        pointops_cuda.precompute_index_pairs_cuda(
            N,
            n,
            k,
            p2v_map.int(),
...
class PrecomputeOffsets(Function):
    ...
    def forward(ctx, N, n, n_max, counts):
        ...
        pointops_cuda.precompute_offsets_cuda(
            N,
            n,
            n_max,
            counts.int(),
...
class PrecomputeAll(Function):
    ...
    def forward(ctx, N, n, n_max, counts):
        ...
        pointops_cuda.precompute_all_cuda(
            N,
            n,
            n_max,
            counts.int(),
...
class PrecomputeIndexPairsQK(Function):
    ...
    def forward(ctx, p2v_map, p2v_map2, counts, counts2):
        ...
        assert (
            p2v_map.is_contiguous()
            and p2v_map2.is_contiguous()
            and counts.is_contiguous()
            and counts2.is_contiguous()
        )
        n, k = p2v_map.shape
        k2 = p2v_map2.shape[1]
        counts_out = counts * counts2
        out_offsets = torch.cat(
            [torch.tensor([0]).cuda().long(), counts_out], 0
        ).cumsum(-1)
        ...
        index_0 = torch.cuda.IntTensor(M).zero_()
        index_1 = torch.cuda.IntTensor(M).zero_()
        pointops_cuda.precompute_index_pairs_q_k_cuda(
            N,
            n,
            k,
            k2,
...
class PrecomputeIndexPairsV2(Function):
    ...
    def forward(ctx, p2v_map_down, counts_down, p2v_map, counts, index_0_offsets):
        ...
        """
        assert (
            p2v_map_down.is_contiguous()
            and counts_down.is_contiguous()
            and p2v_map.is_contiguous()
            and counts.is_contiguous()
            and index_0_offsets.is_contiguous()
        )
        n, k = p2v_map.shape
        ...
        index_0 = torch.cuda.IntTensor(M).zero_()
        index_1 = torch.cuda.IntTensor(M).zero_()
        pointops_cuda.precompute_index_pairs_cuda_v2(
            N,
            n,
            k_down,
            k,
    ...
    def forward(ctx, q, k, index1, index0_offsets, n_max):
        ...
        if n_max <= 1024:
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "attn_step1_v2: forward time: {}s, M: {}".format(time.time() - t, M)
                )
        else:
            ...
            pointops_cuda.attention_step1_forward_cuda_v6(
                N_k, M, h, C, n_max, q, k, index0_offsets, index1, output
            )
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "attn_step1_v6: forward time: {}s, M: {}".format(time.time() - t, M)
                )
    ...
    def backward(ctx, grad_output):
        ...
        if n_max <= 1024:
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "attn_step1_v2: backward time: {}s, M: {}".format(
                        time.time() - t, M
                    )
                )
                # input()
            ...
            pointops_cuda.attention_step1_backward_cuda_v6(
                N_q,
                M,
                h,
                C,
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "attn_step1_v6: backward time: {}s, M: {}".format(
                        time.time() - t, M
                    )
                )
                # input()
...
class AttentionStep1_v7(Function):
    ...
    def forward(ctx, q, k, index1, index1_offsets, index0, index0_offsets, n_max):
        ...
        pointops_cuda.attention_step1_forward_cuda_v7(
            N_k, M, h, C, n_max, q, k, index0_offsets, index1, output
        )
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.attention_step1_backward_cuda_v7(
            N_q,
            M,
            h,
            C,
...
class AttentionStep1_v8(Function):
    ...
    def forward(ctx, q, k, index1, index1_offsets, index0, index0_offsets, n_max):
        ...
        pointops_cuda.attention_step1_forward_cuda_v7(
            N_k, M, h, C, n_max, q, k_transpose, index0_offsets, index1, output
        )
...
class AttentionStep1_v9(Function):
    ...
    def forward(ctx, q, k, index1, index1_offsets, index0, index0_offsets, n_max):
        ...
        pointops_cuda.attention_step1_forward_cuda_v9(
            N_k, M, h, C, n_max, q, k, index0, index1, output
        )
...
class AttentionStep1_v3(Function):
    ...
    def forward(ctx, q, k, index0, index1, index0_offsets, n_max):
        ...
        output = torch.cuda.FloatTensor(M, h).zero_()
        pointops_cuda.attention_step1_forward_cuda_v3(
            N,
            M,
            h,
            C,
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.attention_step1_backward_cuda_v3(
            N,
            M,
            h,
            C,
...
class AttentionStep1_v4(Function):
    ...
    def forward(ctx, q, k, index1, index0_offsets, n_max):
        ...
        print(
            "index0_offsets: {}, N_k: {}, index0_offsets.shape: {}".format(
        ...
        output = torch.cuda.FloatTensor(M, h).zero_()
        pointops_cuda.attention_step1_forward_cuda_v4(
            N_k, M, h, C, n_max, q, k, index0_offsets, index1, output
        )
        ctx.N_q = N_q
        ctx.N_k = N_k
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.attention_step1_backward_cuda_v4(
            N_q,
            M,
            h,
            C,
...
class AttentionStep1_v5(Function):
    ...
    def forward(ctx, q, k, p2v_map, counts, n_max):
        ...
        M = offset[-1].item()
        output = torch.cuda.FloatTensor(M, h).zero_()
        pointops_cuda.attention_step1_forward_cuda_v5(
            N_k, M, h, C, n, n_max, q, k, p2v_map, counts, offset.int(), output
        )
        ctx.N_q = N_q
        ctx.N_k = N_k
        ctx.C = C
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.attention_step1_backward_cuda_v5(
            N_q,
            M,
            h,
            C,
...
class AttentionStep2_v7(Function):
    ...
    def forward(ctx, attn, v, index0, index0_offsets, n_max, index1, index1_offsets):
        ...
        # pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v4(N, M, h, hdim, n_max, attn, v, index0_offsets, index1, table, rel_idx, output)
        pointops_cuda.attention_step2_forward_cuda_v7(
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_v7: forward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.attention_step2_backward_cuda_v7(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_v7: backward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
...
class DotProdWithIdx_v3(Function):
    ...
    def forward(ctx, q, index_q_offsets, n_max, k, index_k, table_q, table_k, rel_idx):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx_v3: forward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
    ...
    def backward(ctx, grad_output):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx_v3: backward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
...
class DotProdWithIdx_v7(Function):
    ...
    def forward(
        ...
        pointops_cuda.dot_prod_with_idx_forward_cuda_v7(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx_v7: forward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
    ...
    def backward(ctx, grad_output):
        ...
        table_k = table_k.permute(
            0, 3, 1, 2
        ).contiguous()  # [L, h, hdim, 3] -> [L, 3, h, hdim]
        # torch.cuda.synchronize()
        # start = time.time()
        ...
        pointops_cuda.dot_prod_with_idx_backward_cuda_v7(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx_v7: backward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
...
class DotProdWithIdx_v8(Function):
    ...
    def forward(
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx_v8: forward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
    ...
    def backward(ctx, grad_output):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx_v8: backward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
...
class DotProdWithIdx_v9(Function):
    ...
    def forward(
        ...
        # pointops_cuda.dot_prod_with_idx_forward_cuda(N, M, h, hdim, q, index, table, rel_idx, output)
        # pointops_cuda.dot_prod_with_idx_forward_cuda_v3(N, M, h, hdim, n_max, q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, output)
        pointops_cuda.dot_prod_with_idx_forward_cuda_v9(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx_v9: forward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
...
class DotProdWithIdxAll_v9(Function):
    ...
    def forward(
        ...
        # pointops_cuda.dot_prod_with_idx_forward_cuda(N, M, h, hdim, q, index, table, rel_idx, output)
        # pointops_cuda.dot_prod_with_idx_forward_cuda_v3(N, M, h, hdim, n_max, q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, output)
        pointops_cuda.dot_prod_with_idx_all_forward_cuda_v9(
            N,
            M,
            h,
            hdim,
...
class DotProdWithIdxAll_v10(Function):
    ...
    def forward(
        ...
        # pointops_cuda.dot_prod_with_idx_forward_cuda(N, M, h, hdim, q, index, table, rel_idx, output)
        # pointops_cuda.dot_prod_with_idx_forward_cuda_v3(N, M, h, hdim, n_max, q, index_q_offsets, k, index_k, table_q, table_k, rel_idx, output)
        pointops_cuda.dot_prod_with_idx_all_forward_cuda_v10(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx_v10: forward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
...
class DotProdWithIdx_v5(Function):
    ...
    def forward(ctx, q, index_q_offsets, n_max, k, index_k, table_q, table_k, rel_idx):
        ...
        if n_max <= 1024:
            ...
            # pointops_cuda.dot_prod_with_idx_forward_cuda(N, M, h, hdim, q, index, table, rel_idx, output)
            pointops_cuda.dot_prod_with_idx_forward_cuda_v6(
                N,
                M,
                h,
                hdim,
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "dot_prod_with_idx_v6: forward time: {}s, M: {}".format(
                        time.time() - t, M
                    )
                )
    ...
    def backward(ctx, grad_output):
        ...
        if n_max <= 1024:
            ...
            pointops_cuda.dot_prod_with_idx_backward_cuda_v5(
                N,
                M,
                h,
                hdim,
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "dot_prod_with_idx_v5: backward time: {}s, M: {}".format(
                        time.time() - t, M
                    )
                )
        else:
            ...
            pointops_cuda.dot_prod_with_idx_backward_cuda_v6(
                N,
                M,
                h,
                hdim,
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "dot_prod_with_idx_v6: backward time: {}s, M: {}".format(
                        time.time() - t, M
                    )
                )
...
class DotProdWithIdx_v4(Function):
    @staticmethod
    def forward(
        ctx, q, index_q, index_q_offsets, n_max, k, index_k, table_q, table_k, rel_idx
    ):
        """
        input: q: (N, h, hdim), index_q: (M), k: (N, h, hdim), index_k: (M), table_q: (L, h, hdim, 3), table_k: (L, h, hdim, 3), rel_idx: (M, 3)
        output: output: [M, h]
        ...
        output = torch.cuda.FloatTensor(M, h).zero_()
        # pointops_cuda.dot_prod_with_idx_forward_cuda(N, M, h, hdim, q, index, table, rel_idx, output)
        pointops_cuda.dot_prod_with_idx_forward_cuda_v4(
            N,
            M,
            h,
            hdim,
        ...
        # ctx.T = T
        ctx.save_for_backward(
            q, index_q, index_q_offsets, k, index_k, table_q, table_k, rel_idx
        )
        return output
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.dot_prod_with_idx_backward_cuda_v4(
            N,
            M,
            h,
            hdim,
...
class AttentionStep2WithRelPosValue_v2(Function):
    ...
    def forward(ctx, attn, v, index0_offsets, n_max, index1, table, rel_idx):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value_v2: forward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
    ...
    def backward(ctx, grad_output):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value_v2: backward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
...
class AttentionStep2WithRelPosValue_v4(Function):
    ...
    def forward(ctx, attn, v, index0_offsets, n_max, index1, table, rel_idx):
        ...
        if n_max <= 1024:
            ...
            pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v4(
                N,
                M,
                h,
                hdim,
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "attn_step2_with_rel_pos_value_v4: forward time: {}s, M: {}, h: {}".format(
                        time.time() - t, M, h
                    )
                )
        else:
            ...
            pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v6(
                N,
                M,
                h,
                hdim,
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "attn_step2_with_rel_pos_value_v6: forward time: {}s, M: {}, h: {}".format(
                        time.time() - t, M, h
                    )
                )
    ...
    def backward(ctx, grad_output):
        ...
        if n_max <= 1024:
            ...
            pointops_cuda.attention_step2_with_rel_pos_value_backward_cuda_v4(
                N,
                M,
                h,
                hdim,
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "attn_step2_with_rel_pos_value_v4: backward time: {}s, M: {}, h: {}".format(
                        time.time() - t, M, h
                    )
                )
        else:
            ...
            pointops_cuda.attention_step2_with_rel_pos_value_backward_cuda_v6(
                N,
                M,
                h,
                hdim,
            ...
            if DEBUG:
                torch.cuda.synchronize()
                print(
                    "attn_step2_with_rel_pos_value_v6: backward time: {}s, M: {}, h: {}".format(
                        time.time() - t, M, h
                    )
                )
...
class AttentionStep2WithRelPosValue_v7(Function):
    ...
    def forward(
        ...
        # pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v4(N, M, h, hdim, n_max, attn, v, index0_offsets, index1, table, rel_idx, output)
        pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v7(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value_v7: forward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.attention_step2_with_rel_pos_value_backward_cuda_v7(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value_v7: backward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
...
class AttentionStep2WithRelPosValue_v7p1(Function):
    ...
    def forward(
        ...
        # pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v4(N, M, h, hdim, n_max, attn, v, index0_offsets, index1, table, rel_idx, output)
        pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v8(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value_v8: forward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
...
class AttentionStep2WithRelPosValue_v10(Function):
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.attention_step2_with_rel_pos_value_backward_cuda_v10(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value_v10: backward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
        ...
        grad_table = grad_table.permute(
            3, 0, 1, 2
        ).contiguous()  # [h, hdim, 3, L] -> [L, h, hdim, 3]
...
class AttentionStep2WithRelPosValue_v8(Function):
    ...
    def backward(ctx, grad_output):
        ...
        table = table.permute(
            2, 3, 1, 0
        ).contiguous()  # [L, 3, h, hdim] -> [h, hdim, 3, L]
        v = v.permute(1, 2, 0).contiguous()
        rel_idx = rel_idx.permute(1, 0).contiguous()
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value_v8: backward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
...
class AttentionStep2WithRelPosValue_v9(Function):
    @staticmethod
    def forward(
        ctx, attn, v, index0, index0_offsets, n_max, index1, index1_offsets, pos_emb_v
    ):
        """
        ...
        # pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v4(N, M, h, hdim, n_max, attn, v, index0_offsets, index1, table, rel_idx, output)
        pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v9(
            N, M, h, hdim, n_max, attn, v, index0_offsets, index1, pos_emb_v, output
        )

        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value_v9: forward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
        ...
        ctx.n_max = n_max
        ctx.save_for_backward(
            attn, v, index0, index0_offsets, index1, index1_offsets, pos_emb_v
        )
        return output
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.attention_step2_with_rel_pos_value_backward_cuda_v9(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value_v9: backward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
...
class AttentionStep2WithRelPosValue_v5(Function):
    ...
    def forward(ctx, attn, v, index0_offsets, n_max, index1, table, rel_idx):
        ...
        pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v5(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value_v5: forward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
...
class AttentionStep2WithRelPosValue_v3(Function):
    ...
    def forward(ctx, attn, v, index0_offsets, n_max, index0, index1, table, rel_idx):
        ...
        pointops_cuda.attention_step2_with_rel_pos_value_forward_cuda_v3(
            N,
            M,
            h,
            hdim,
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.attention_step2_with_rel_pos_value_backward_cuda_v3(
            N,
            M,
            h,
            hdim,
