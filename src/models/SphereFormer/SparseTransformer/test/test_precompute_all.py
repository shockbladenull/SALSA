...
v2p_map = torch.IntTensor([1, 0, 0, 2, 0, 2, 2, 1, 2, 2, 2]).cuda()

p2v_map = torch.IntTensor(
    [[1, 2, 4, 0, 0, 0], [0, 7, 0, 0, 0, 0], [5, 6, 3, 9, 8, 10]]
).cuda()
...
index_0_offsets_, index_1_offsets_, index_0_, index_1_ = sptr.precompute_all(
    N, p2v_map.shape[0], p2v_map.shape[1], counts
)
...
print(
    "index_0_offsets.shape: {}, index_0_offsets_.shape: {}".format(
        index_0_offsets.shape, index_0_offsets_.shape
    )
)

print(
    "ctg_index_1_offsets.shape: {}, index_1_offsets_.shape: {}".format(
        ctg_index_1_offsets.shape, index_1_offsets_.shape
    )
)
