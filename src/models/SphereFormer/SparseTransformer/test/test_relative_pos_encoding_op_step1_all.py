...
attn_flat = pointops.attention_step1(
    query.float(), key.float(), index_q.int(), index_k.int()
)

output = output1 + output2 + attn_flat
loss = output.mean()
loss.backward()
...
table_q.requires_grad = True
table_k.requires_grad = True
output_v2 = sptr.dot_prod_with_idx_all(
    query,
    index_q.int(),
    index_q_offsets.int(),
    n_max,
...
print("output[:5, :5]: ", output[:5, :5])

print("output_v2[:5, :5]: ", output_v2[:5, :5])
...
print(
    "((table_q_grad2-table_q_grad)**2).max(): ",
    ((table_q_grad2 - table_q_grad) ** 2).max(),
)

print(
    "((table_k_grad2-table_k_grad)**2).max(): ",
    ((table_k_grad2 - table_k_grad) ** 2).max(),
)
